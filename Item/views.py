...
class ItemDetail(View):
    ...
    def post(self, request, id):
        ...
        if not self.request.user.is_authenticated:
            ...
            feedb = Feedback.objects.create(
                owner=owner, user=user, item=item, experience=experience, feedback=feedback)
            feedb.save()
    ...
    def post(self, request):
        if request.user.is_authenticated:
            ...
            if request.method == 'POST':
                ...
                item = Item.objects.create(category=category, user=user, title=title, image=image, keywords=keywords,
                                           description=description, disc=discount, price=price, brand=brand, model=model, color=color, condition=condition)
                ...
                messages.error(
                    request, 'Ooops, there is an error occuring in your process please contact the Admin!!!')
        else:
            return redirect('account_login')
...
class UserMessageView(View):
    ...
    def post(self, request):
        ...
        if request.method == 'POST':
            ...
            send_chat = ChatMessage.objects.create(
